...
# @Time    : 2018/4/23 9:39
"""
code function: some functions about paper.
"""
...
def get_paper_dblp(connection):
    ...
    try:
        ...
        for result in results:
            conf = re.findall(r".*/(.*?)/.*", result[0])[0].strip()
            paper_dblps[result[0]] = PaperDblp(result[0], result[1].lower(), conf, result[2])
            all_title += result[1].lower() + "\n"
    except Exception as e:
        print("Error: ", repr(e))
    return paper_dblps, all_title
    ...
    with open(matched_file_name, 'r', encoding='utf-8') as f:
        ...
        for result in results:
            ...
            if len(temp) == 2:
                ...
                matched_papers_dict[temp[0].lower().strip()] = [elem for elem in temp[1].lower().strip().split(",") if elem != '']
