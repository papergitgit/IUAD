...
class FellegiSunter():
    ...
    def paper_linkageEF(self, pairs_paper):
        '''
        :param pairs_paper: pairs_paper with weight.
    ...
    def test_linkage_single(self, inferred, pair_test_elem):
        max_decision = -1000000
        max_author_new = None
        for i in range(0, len(pair_test_elem)):
            temp = pair_test_elem[i][0]
            author_new = pair_test_elem[i][1]
            weight = 0
            sim = temp.get_similarity()   # double list
            ...
            if weight >= max_decision:
                max_decision = weight
                max_author_new = author_new

        return max_author_new, max_decision
            ...
            for j in range(0, len(sim)):
                if self.dist[j] == "bernoulli":
                    ...
                    if sim[j] >= 0:
                        ...
                        exp.set_lambda1(exp.get_lambda1() + 1.0 / before.get_exponential_map().get(j).get_lambda1() * expectation.get[i])
                        exp.set_lambda2(exp.get_lambda2() + 1.0 / before.get_exponential_map().get(j).get_lambda2() * (1 - expectation.get[i]))
